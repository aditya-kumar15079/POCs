...
const EvaluationForm = ({ id }) => {
  ...
  const debouncedUpdateFileContent = React.useMemo(() => debounce(updateFileContent, 5000), []);
  ...
  return (
    <>
          ...
          {Object.entries(fileConfig?.evaluation?.metrics || {}).map(([label, value]) => (
            <div key={label} className="flex items-center justify-between">
              ...
              <label className="inline-flex items-center cursor-pointer">
                ...
                <div className={`w-10 h-6 rounded-full transition ${value ? "bg-blue-600" : "bg-gray-300"}`}></div>
              </label>
            </div>
          ))}
        </div>
      ...
      <div className="flex justify-end">
        <button className="bg-blue-600 text-white px-4 py-2 rounded mt-4" onClick={executeTest}>
          Execute Test
        </button>
      </div>
    </>
